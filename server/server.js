...
app.get(`/qa/questions`, async (req, res) => {
    ...
    to_timestamp(date_written/1000) as question_date, asker_name, helpful as question_helpfulness,
     reported FROM questions WHERE product_id = ${id} AND reported = 'false' LIMIT 50`)
        .then(output => {
            ret.results = output.rows;
            return ret
        })
...
app.put('/qa/questions/:question_id/report', async (req, res) => {
    ...
    return pool.query(`update questions set reported = ${!count} where id = ${question_id}`)
        .then(output => {
...
app.put('/qa/answers/:answer_id/report', async (req, res) => {
    ...
    count = count.rows[0].reported;
    return pool.query(`update questions set reported = ${!count} where id = ${answer_id}`)
        .then(output => {
